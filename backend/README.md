...
6. Set up Tailscale funnel for public access:
   - Install Tailscale on Pi if not already.
   - Run: `tailscale funnel 5000`
   - Get the public URL (e.g., https://pi-name.ts.net)

7. Update `index.html` on the frontend with the funnel URL.

8. Deploy the frontend to GitHub Pages as before.

The backend will use Tor for all outgoing requests to DDGS for security.